...
/*   Updated: 2024/03/21 11:52:31 by aaleksee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buffer[fd])
		return (NULL);
	result = get_line1(&buffer[fd], &res_len);
	if (bytes_read != 0)
		buffer[fd] = truncate_buffer(buffer[fd], res_len);
	return (result);
}
...
char	*append_to_buffer(char *buffer, int fd, ssize_t **bytes_read)
{
	...
	**bytes_read = read(fd, read_buffer, BUFFER_SIZE);
	...
	ft_memcpy(buffer, temp_buffer, temp_buffer_len);
	free(temp_buffer);
	return (buffer);
}

char	*get_line1(char **buffer, size_t *len)
{
	char	*newline;
	char	*res;
